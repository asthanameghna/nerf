...
def init_nerf_relight_model(D=8, W=256, input_ch=3, input_ch_views=3, input_ch_lights=3, output_ch=4, skips=[4], use_viewdirs=False, use_lightdirs=False):
    ...
    if (use_viewdirs==True) and (use_lightdirs==False):
        ...
        for i in range(2): #changed from 1
        ...
        alpha_out = dense(1, act=None)(outputs)
        bottleneck = dense(8, act=None)(outputs) # testing out new theory
...
def train():
    ...
    light_dirs = np.load('data/nerf_synthetic/lego/lightdirs_trainLEGO.npy',)
    ...
    for i in range(start, N_iters):
        ...
        #         rays_o, rays_d, light_d = [get_rays_relight_np(H, W, focal, p, l) for p,l in zip(poses, light_dirs)]

...
def init_nerf_relight_model(D=8, W=256, input_ch=3, input_ch_views=3, input_ch_lights=3, output_ch=4, skips=[4], use_viewdirs=False, use_lightdirs=False):
    ...
    input_ch_hvec = int(input_ch_views+input_ch_lights)

    inputs = tf.keras.Input(shape=(input_ch + input_ch_views + input_ch_lights + input_ch_hvec))
    inputs_pts, inputs_views, inputs_lights, inputs_hvec = tf.split(inputs, [input_ch, input_ch_views, input_ch_lights, input_ch_hvec], -1)
    inputs_pts.set_shape([None, input_ch])
    inputs_views.set_shape([None, input_ch_views])
    inputs_lights.set_shape([None, input_ch_lights])
    a
    inputs_hvec.set_shape([None, input_ch_hvec])

    print(inputs.shape, inputs_pts.shape, inputs_views.shape, inputs_lights.shape, inputs_hvec.shape)
    outputs = inputs_pts
    for i in range(D):
    ...
    if (use_viewdirs==True) and (use_lightdirs==False):
        ...
        bottleneck = dense(6, act=None)(outputs)
    ...
    elif (use_viewdirs==False) and (use_lightdirs==True):
        alpha_out = dense(1, act=None)(outputs)
        bottleneck = dense(6, act=None)(outputs)
        inputs_lightdirs = tf.concat(
            [bottleneck, inputs_lights], -1)  # concat lightdirs
        outputs = inputs_lightdirs
        # The supplement to the paper states there are 4 hidden layers here, but this is an error since
        # the experiments were actually run with 1 hidden layer, so we will leave it as 1.
        for i in range(4): #changed from 1
            outputs = dense(W//2)(outputs)
        ...
        alpha_out = dense(1, act=None)(outputs)
        bottleneck = dense(6, act=None)(outputs) # testing out new theory
        inputs_viewlightdirs = tf.concat(
            [bottleneck, inputs_views, inputs_lights, inputs_hvec], -1)  # concat viewdirs and lightdirs
        outputs = inputs_viewlightdirs
        # The supplement to the paper states there are 4 hidden layers here, but this is an error since
        # the experiments were actually run with 1 hidden layer, so we will leave it as 1.
        for i in range(4):
            outputs = dense(W//2)(outputs)
...
def run_network_relight(inputs, viewdirs, lightdirs, fn, embed_fn, embeddirs_fn_views, embeddirs_fn_lights, netchunk=1024*64):
    """Prepares inputs and applies network 'fn'."""
    ...
    if (viewdirs is not None) and (lightdirs is not None):
        ...
        embedded_dirs_view = embeddirs_fn_views(input_dirs_flat_view)
        embedded = tf.concat([embedded, embedded_dirs_view], -1)
        ...
        embedded_dirs_light = embeddirs_fn_lights(input_dirs_flat_light)
        embedded = tf.concat([embedded, embedded_dirs_light], -1)
...
def render_relight(H, W, focal, lightdir=None,
    ...
    # if use_viewdirs:
    #     # (ray origin, ray direction, light direction, min dist, max dist, normalized viewing direction)
...
def render_relight_path_multilight(render_poses, lightdirs, hwf, chunk, render_kwargs, gt_imgs=None, savedir=None, render_factor=0):
...
def create_nerf_relight(args):
    ...
    #     embeddirs_fn, input_ch = get_embedder(
    #         args.multires_views, args.i_embed)

    if (args.use_viewdirs==True) and (args.use_lightdirs==True):
        embeddirs_fn_views, input_ch_views = get_embedder(
            args.multires_views, args.i_embed)
        embeddirs_fn_lights, input_ch_lights = get_embedder(
            args.multires_views, args.i_embed)
    ...
    model = init_nerf_relight_model(
        ...
        input_ch_views=input_ch_views, input_ch_lights=input_ch_lights, use_viewdirs=args.use_viewdirs, use_lightdirs=args.use_lightdirs)
    print(model)
    # tf.keras.utils.plot_model(model, "relight_nerf_arch.png", show_shapes=True)
...
def config_parser():
    ...
    parser.add_argument("--lightdir", type=int, default=[-0.80901699, 0.5, -0.30901699],
...
def train():
    ...
    light_dirs = np.load('data/nerf_llff_data/mini_relight_bunny/minilightdirs.npy',)
    ...
    for i in range(start, N_iters):
        ...
        if use_batching:
            ...
            batch_rays, target_s = batch[:3], batch[3] # change this from batch[:2] to batch[:3] to include light direction & batch[2] to batch[3] as rgb moved further
        ...
        if i % args.i_testset == 0 and i > 0:
            ...
            if args.multilightdirs:
                multilights = np.load('data/nerf_llff_data/mini_relight_bunny/multilights.npy',)
                render_relight_path_multilight(poses[i_test], multilights, hwf, args.chunk, render_kwargs_test,
                                    gt_imgs=images[i], savedir=testsavedir)
            else:

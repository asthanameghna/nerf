...
def init_nerf_relight_model(D=8, W=256, input_ch=3, input_ch_views=3, input_ch_lights=3, output_ch=4, skips=[4], use_viewdirs=False, use_lightdirs=False):
    ...
    inputs = tf.keras.Input(shape=(input_ch + input_ch_views + input_ch_lights))
    ...
    if (use_viewdirs==True) and (use_lightdirs==False):
        ...
        alpha_out = dense(1, act=None)(outputs)
        bottleneck = dense(16, act=None)(outputs)
        inputs_viewlightdirs = tf.concat(
            [bottleneck, inputs_views, inputs_lights], -1)  # concat viewdirs and lightdirs
...
def render_relight_path(render_poses, lightdir, hwf, chunk, render_kwargs, gt_imgs=None, savedir=None, render_factor=0, **kwargs):
...
def render_relight_path_multilight(render_poses, lightdirs, hwf, chunk, render_kwargs, gt_imgs=None, savedir=None, render_factor=0, **kwargs):
...
def render_relight_path_multilight_onepose(render_pose, lightdirs, hwf, chunk, render_kwargs, gt_imgs=None, savedir=None, render_factor=0, **kwargs):
...
def config_parser():
    ...
    parser.add_argument("--lightdir", type=int, default=[-0.0571, -0.2123, 0.9755],
...
def train():
    ...
    light_dirs = np.load('data/nerf_llff_data/readingPNG_s2_bmask/lightdirs.npy',)
    ...
    for i in range(start, N_iters):
        ...
        if i % args.i_testset == 0 and i > 0:
            ...
            if args.multilightdirs:
                multilights = np.load('data/nerf_llff_data/readingPNG_s2_bmask/multilights_z2.npy',)
                # render_relight_path_multilight(poses[i_test], multilights, hwf, args.chunk, render_kwargs_test,
                #                     gt_imgs=images[i_test], savedir=testsavedir)
                i_test_1 = i_test[0]
                render_relight_path_multilight_onepose(poses[i_test_1], multilights, hwf, args.chunk, render_kwargs_test,
